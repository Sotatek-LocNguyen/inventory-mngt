# Phase 01 — Write and Register Seed Script

**Objective:** Create `apps/api/prisma/seed.ts` with idempotent fried chicken demo data and register it in `package.json`.

---

## Task 1.1 — Create `apps/api/prisma/seed.ts`
...
## Task 1.2 — Register seed in `apps/api/package.json`

Add a `prisma.seed` field and a `seed` script:
...
- `apps/api/prisma/seed.ts` exists and has no syntax errors
- `apps/api/package.json` contains `prisma.seed` and `seed` script
